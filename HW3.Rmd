...
# 8
a. False, as $\alpha$ increases, the power increases. ??
...
c. True, we can see that on the F-table. ??
...
MSE = anova.table[2,3]
alpha = 0.05
the.power = give.me.power(group.means,group.nis,MSE,alpha)
```
a. The power is $`r round(the.power, 4)`$.
```{r}
...
n = test$n
```
b. The smallest sample size we need for each group is $n_i = `r ceiling(n)`$.
```{r}

# I-c
test = pwr.anova.test(k = 3, f = effect.size, sig.level = 0.01, power = 0.99)
n = test$n
```
c. The smallest sample size we need for each group is $n_i = `r ceiling(n)`$.
d. Yes, because the power of our test increases as our $\phi$ increases, which is positively correlated with our F-statistic due to $SSA$ being in the numerator and $MSE$ being in the denominator of $\phi$.
...
d. The results do not suggest a significant difference between the average time of colon and the average time of both stomach and bronchus because zero is in the interval.
