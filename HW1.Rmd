...
<br>$\sigma^2[\frac{\bar{Y_1} + \bar{Y_2}}{2} - \bar{Y_3}] = \sqrt{\frac{\frac{\sigma_1^2}{100}+\frac{\sigma_2^2}{100}}{4} + \frac{\sigma_3^2}{100}}$
...
<br>$\sigma^2[\bar{Y_1} + \bar{Y_3} - 2 \bar{Y_2}] = \sqrt{\frac{\sigma_1^2}{100} + \frac{\sigma_3^2}{100} - 4 * \frac{\sigma_2^2}{100}}$
...
<br> Sample difference in mean is 4.4037 standard deviations away from the null hypothesis.
c. $p < 0.0005$
...
## 6
a. Type I error would be incorrectly rejecting the null hypothesis, where we would fail to state that average systolic blood pressure is the same across smoking statuses when it is.
b. Type II error is to fail to reject the null hypothesis, where we would fail to state that average systolic blood pressure differs between smoking statuses when it is.
c. $t = \pm 2.585718$
<br> Confidence interval is [4.479227, 17.22077].
d. The confidence interval means that we are 99% certain the difference between the two means is between 4.479227 and 17.22077.
e. The largest difference we expect between the two groups with 99% confidence is 17.22077. This means that we expect the mean of non-smokers to be at most 17.22077 less than the mean of smokers.
...
## I
a. Low Stress had the highest average.
b. Yes, the standard deviations are approximately equal.
c. Medium exercise has the lowest average age.
d. Low exercise group differs most from its mean as it has the highest standard deviation.

## II
a. There does not seem to be a trend as the boxplots have approximately the same means, lower quartiles, and upper quartiles.
b. Both groups vary around the same amount since the range of data is around the same.
c. Medium exercise has the highest proportion of high stress subjects.
d. The has marriage group has the highest proportion of low stress subjects.

## III
a. Low exercise group has the most subjects.
b. Middle stress group has the highest age.
c. Middle stress group has the highest average age.
d. Females have the lowest average systolic blood pressure.

## IV
a. $t_s = 1.352333$
b. $p = 0.1768947$
c. $[-1.537372, 8.326066]$
d. The confidence interval tells us the range one mean lies within the other on a normal distribution.
e. ??
