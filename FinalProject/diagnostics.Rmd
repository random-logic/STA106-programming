...
bf_test <- function(data) {
  model <- aov(Y ~ A * B, data = data)
  return(car::leveneTest(model$residuals ~ paste(A, B),
                         data = data, center = median)[1, 3])
}
...
res$SW_Test_P_Value <- apply(res, 1, function(row) {
  return(sw_test(row["data"][[1]]))
})

res$Levene_Test_P_Value <- apply(res, 1, function(row) {
  return(bf_test(row["data"][[1]]))
})

res <- res[, -which(names(res) == "data")]

knitr::kable(res, digits = 16)
```
