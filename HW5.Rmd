...
<br> $p = `r round(the.p.value, 4)`$
<br> Since $p \leq \alpha$, we reject $H_0$. Therefore, there is an interaction effect and we should use the full model as follows:
<br> $Y_{ijk} = \mu_{..} + \gamma_i + \delta_j + (\gamma\delta)_{ij} + \epsilon_{ijk}$
b. $R^2\{A|\cdot\} = `r round(R_squared[["A|N"]], 4)`$
<br> $R^2\{B|\cdot\} = `r round(R_squared[["B|N"]], 4)`$
