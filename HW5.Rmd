...
d. $R^2\{AB|(A+B)\} = `r round(R_squared[["AB|(A+B)"]], 4)`$
   Our value seems reasonable because we concluded that there is no interaction effect which is typical for small values of $R^2$ (close to 0).
...
a. $H_0:$ All $\gamma_i = 0$
   ...
   Since $p \leq \alpha$, we reject $H_0$. Therefore, there is a factor A effect.
...
b. $H_0:$ All $\gamma_i = 0$
   ...
   Since $p \leq \alpha$, we reject $H_0$. Therefore, there is a factor B effect.
...
c. $R^2\{(A+B)|B\} = `r round(R_squared[["(A+B)|B"]], 4)`$
   ...
   $R^2\{(A+B)|A\} = `r round(R_squared[["(A+B)|A"]], 4)`$
...
MSE <- SSE_F / df_SSE_F
```
e. $\sigma_\epsilon^2 = `r round(MSE, 4)`$
...
c. True, we can see this in the equation as follows:
   ...
   $R^2 = \frac{SSE_F - SSE_R}{SSE_F}$
  
   In the equation, we are assuming that the full model always has more unexplained variance ($SSE$) than the reduced model. Hence, the numerator is always less than or equal to the denominator.
...
b. $SSE\{AB\} = `r round(SSE["SSE", "AB"], 4)`$
   ...
   $SSE\{A + B\} = `r round(SSE["SSE", "(A+B)"], 4)`$
c. The table of all $\bar Y_{ij}$ is as follows:
   `r knitr::kable(means, format = 'markdown')`
...
b. $\bar Y_{H.} - \bar Y_{L.} = `r round(diff.means_A, 4)`$
   ...
   $\bar Y_{.B} - \bar Y_{.P} = `r round(diff.means_B, 4)`$

c. $F_s = `r round(the.f.statistic_A, 4)`$

   $p = `r round(the.p.value_A, 4)`$

   Since $p \leq \alpha$, we reject $H_0$. Therefore, there is a factor A effect.

d. $F_s = `r round(the.f.statistic_B, 4)`$

   $p = `r round(the.p.value_B, 4)`$

   Since $p > \alpha$, we fail to reject $H_0$. Therefore, there is no factor B effect.
...
b. $SSE\{AB\} = `r round(SSE["SSE", "AB"], 4)`$
   ...
   $SSE\{A\} = `r round(SSE["SSE", "A"], 4)`$

   $SSE\{B\} = `r round(SSE["SSE", "B"], 4)`$

   $SSE\{Empty\} = `r round(SSE["SSE", "Empty/Null"], 4)`$
...
```
a. $F_s = `r round(the.f.statistic, 4)`$

   $p = `r round(the.p.value, 4)`$

   Since $p \leq \alpha$, we reject $H_0$. Therefore, there is an interaction effect and we should use the full model as follows:
   $Y_{ijk} = \mu_{..} + \gamma_i + \delta_j + (\gamma\delta)_{ij} + \epsilon_{ijk}$

b. $R^2\{A|\cdot\} = `r round(R_squared[["A|N"]], 4)`$

   $R^2\{B|\cdot\} = `r round(R_squared[["B|N"]], 4)`$
