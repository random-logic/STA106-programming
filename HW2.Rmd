...
<br> $\to 0$
d. $E[\bar{Y}_{..}] = \frac{\sum_i \bar{Y}_{i.}}{n_T} = \frac{\sum_i \sum_j \bar{Y}_{ij}}{n_T} = \frac{\sum_i n_i \mu_i}{n_T} = \mu_.$
e. $\sigma^2(Y_{ij}) = \sigma^2(\mu_i) + \sigma^2(\epsilon) = 0 + \sigma^2_\epsilon = \sigma^2_\epsilon$
...
boxplot(Survival ~ Organ, data = data, 
        ...
        border = "black")
```
b. The averages are displayed in the table below. I believe that we will reject the single factor ANOVA because the mean of Breast is far from the mean of Bronchus relative to the variance. ?? nice formatted table?
```{r}
